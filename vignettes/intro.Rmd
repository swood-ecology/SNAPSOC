...
knitr::opts_knit$set(root.dir = "~/Library/CloudStorage/Box-Box/Work/The Nature Conservancy/Regional Teams/Africa/NRT Soil Carbon/SNAP")
```
...
```{r MC-cost}
cost <- snap(
  input_table = input_table, nsamp = 1000,
...
```{r MC-nocost}
nocost <- snap(
  input_table = input_noprice, nsamp = 1000,
...
Once we have this data file, we can then calculate the SNAP model based on the given data.
```{r SNAP}
# Calculate SNAP outputs
SNAPSOC::snap_data(input = data)
```
